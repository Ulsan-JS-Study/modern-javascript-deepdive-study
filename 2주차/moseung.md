...
Hihi.good();// static 메소드는 위처럼 생성자 없이 바로 호출가능 객체의 프로퍼티에 접근하듯
```
...
```
보통 객체내에 함수 프로퍼티를 호출할떄 person.xxxx()하면서 실행해주는데 
...
private 필드의 선두에 #을 붙여 사용, java의 private과 같은 맥락
```javascript=
class Person {
    //private 필드 정의
    #name = '';
...
class Tries extends Try{
  
}

const tries = new Tries();
console.log(tries.test); //"test
tries.gogo()// "gogo"
```
...
class Tries extends Try{
  constructor(input){
  super(input);  
  console.log(this.test)//tests 여기서 super가 없으면 this참조가 constructor에서 불가능하다.
  }
...
const trying = new Try("test");
const tries = new Tries("tests");
console.log(tries.test);//tests
tries.gogo()
